...
#     if not permission_settings.get("item_permission"):
#         return ""
#     if not user:
...
#         return f"""
#             EXISTS (
#                 SELECT 1 FROM `tabItem Branch` AS d1
#                 WHERE d1.parent = `tabItem`.name 
#                 AND d1.branch IN ({allowed_branches_str})
#                 AND EXISTS (
...
#         allowed_branches_str = ", ".join([f"'{b}'" for b in allowed_branches])
#         return f"EXISTS (SELECT 1 FROM `tabItem Branch` AS d WHERE d.parent = `tabItem`.name AND d.branch IN ({allowed_branches_str}))"
